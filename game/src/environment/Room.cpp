...
Room* Room::GenerateRoom(glm::vec2 position, Side entranceSide, Type type) {
	...
	for (int i = 0; i < 4; i++) {
		...
		std::vector<Engine::BoundingBox> boxes = GetWallBoxes(width, height, hasGap, side);
		wallBoxes.insert(wallBoxes.end(), boxes.begin(), boxes.end());
	}

	// At this point the position of the wall bounding boxes are relative to the centre of the room
...
Room* Room::GenerateHallway(Side entranceSide) {
	...
	// TODO: Get the bounding boxes
}

std::vector<Engine::BoundingBox> Room::GetWallBoxes(int roomWidth, int roomHeight, bool hasGap, Side side) {
